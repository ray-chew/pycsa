...
def validate_chunks(chunks, expected_total_cells=20480):
    ...
    for i in range(len(chunks) - 1):
        ...
        if current_end + 1 != next_start:
            raise ValueError(f"Gap detected: chunk ends at {current_end}, next starts at {next_start}")
...
def merge_chunks(chunks, output_path, datasets_dir):
    ...
    # Create dimensions
    nspec = first_chunk.dimensions['nspec'].size if 'nspec' in first_chunk.dimensions else 100
...
def main():
    ...
    parser.add_argument('--output', type=str, default='icon_etopo_global_FINAL.nc',
