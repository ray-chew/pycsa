...
class GeographicFeature:
    ...
    def __init__(self, name: str, lat_range: Tuple[float, float],
                 lon_range: Tuple[float, float], feature_type: str,
                 validation_func, description: str = ""):
        """
...
def validate_mountain(topo_cell: var.topo_cell, feature: GeographicFeature) -> Dict:
    ...
    message = f"{feature.name}: max elevation {max_elev:.0f}m (expected >{min_expected}m)"
...
def validate_lake(topo_cell: var.topo_cell, feature: GeographicFeature) -> Dict:
    ...
    message = (f"{feature.name}: min elev {min_elev:.0f}m, mean {mean_elev:.0f}m, "
...
def validate_ocean(topo_cell: var.topo_cell, feature: GeographicFeature) -> Dict:
    ...
    mean_depth = -topo_cell.topo[topo_cell.topo < 0].mean() if (topo_cell.topo < 0).any() else 0
    ...
    message = (f"{feature.name}: water fraction {water_fraction:.1%}, "
...
def validate_gulf(topo_cell: var.topo_cell, feature: GeographicFeature) -> Dict:
    ...
    water_fraction = (topo_cell.topo < 0).sum() / topo_cell.topo.size
    mean_water_depth = -topo_cell.topo[topo_cell.topo < 0].mean() if (topo_cell.topo < 0).any() else 0
    ...
    passed = (water_fraction >= min_water_fraction and
              mean_water_depth >= min_expected_depth)
...
def validate_coast(topo_cell: var.topo_cell, feature: GeographicFeature) -> Dict:
    ...
    message = (f"{feature.name}: water {water_fraction:.1%}, land {land_fraction:.1%} "
...
GEOGRAPHIC_FEATURES = [
    # Mountains
    GeographicFeature(
        "Himalayas", (27.0, 30.0), (85.0, 90.0), "mountain",
        validate_mountain,
        "World's highest mountain range (Everest, K2)"
    ),
    GeographicFeature(
        "Andes (Peru)", (-15.0, -10.0), (-77.0, -72.0), "mountain",
        validate_mountain,
        "Andes mountain range in Peru"
    ),
    GeographicFeature(
        "Alps", (45.5, 47.5), (6.0, 11.0), "mountain",
        validate_mountain,
        "European Alps (Mont Blanc)"
    ),
    GeographicFeature(
        "Rockies (Colorado)", (38.0, 41.0), (-108.0, -105.0), "mountain",
        validate_mountain,
    ...
    # Lakes
    GeographicFeature(
        "Lake Superior", (46.5, 48.5), (-89.0, -85.0), "lake",
        validate_lake,
        "Largest Great Lake by area"
    ),
    GeographicFeature(
        "Lake Baikal", (51.5, 55.5), (103.5, 109.5), "lake",
        validate_lake,
        "World's deepest lake in Siberia"
    ),
    GeographicFeature(
        "Lake Titicaca", (-16.5, -15.0), (-69.5, -68.5), "lake",
        validate_lake,
    ...
    # Oceans
    GeographicFeature(
        "Pacific Ocean (mid)", (10.0, 15.0), (-160.0, -150.0), "ocean",
        validate_ocean,
        "Central Pacific Ocean"
    ),
    GeographicFeature(
        "Atlantic Ocean (mid)", (25.0, 30.0), (-50.0, -40.0), "ocean",
        validate_ocean,
    ...
    # Gulfs and Bays
    GeographicFeature(
        "Gulf of Mexico", (27.0, 29.5), (-94.0, -89.0), "gulf",
        validate_gulf,
        "Gulf of Mexico central region with coastal areas"
    ),
    GeographicFeature(
        "Persian Gulf", (26.0, 28.0), (50.0, 52.0), "gulf",
        validate_gulf,
    ...
    # Coasts
    GeographicFeature(
        "California Coast", (35.0, 37.0), (-122.0, -120.0), "coast",
        validate_coast,
        "California coastline near Monterey"
    ),
    GeographicFeature(
        "Mediterranean Coast (Spain)", (40.0, 42.0), (1.0, 3.0), "coast",
        validate_coast,
...
class TestICONETOPOValidation:
    ...
    def load_region_topography(self, setup: Dict, lat_range: Tuple[float, float],
                               lon_range: Tuple[float, float]) -> var.topo_cell:
        """
        ...
        # Load topography
        topo = var.topo_cell()
        etopo_reader = reader.read_etopo_topo(None, params, is_parallel=True, verbose=False)
    ...
    def load_cell_topography(self, setup: Dict, cell_idx: int) -> Tuple[var.topo_cell, np.ndarray, np.ndarray]:
        """
    ...
    def test_topography_data_quality_basic(self, setup):
        ...
        assert topo.lat is not None and topo.lon is not None, "Missing coordinate arrays"
        ...
        assert topo.topo.min() >= -12000, f"Elevation too low: {topo.topo.min()}m (deepest ocean ~-11km)"
        assert topo.topo.max() <= 9000, f"Elevation too high: {topo.topo.max()}m (Everest ~8.8km)"

        print(f"✓ Data quality check passed: shape={topo.topo.shape}, "
    ...
    def test_geographic_feature(self, setup, feature: GeographicFeature):
        ...
        assert result['passed'], f"{feature.name} validation failed: {result['message']}"
    ...
    def test_cell_near_himalayas(self, setup):
        ...
        print(f"  Cell vertices: lat={np.rad2deg(lat_verts)}, lon={np.rad2deg(lon_verts)}")
        print(f"  Topography shape: {topo.topo.shape}")
        print(f"  Elevation: [{topo.topo.min():.0f}, {topo.topo.max():.0f}]m, mean={topo.topo.mean():.0f}m")

        # Verify high elevations
        assert topo.topo.max() > 4000, f"Expected high peaks in Himalayas, got {topo.topo.max():.0f}m"
        assert topo.topo.mean() > 2000, f"Expected high mean elevation, got {topo.topo.mean():.0f}m"
    ...
    def test_cell_in_pacific_ocean(self, setup):
        ...
        assert water_fraction > 0.95, f"Expected mostly water in Pacific, got {water_fraction:.1%}"
        assert topo.topo.mean() < -1000, f"Expected deep ocean, got mean depth {-topo.topo.mean():.0f}m"
    ...
    def test_cell_on_california_coast(self, setup):
        ...
        assert 0.10 < water_fraction < 0.90, f"Expected coastal mix, got {water_fraction:.1%} water"
    ...
    def test_multiple_cells_consistency(self, setup):
        ...
        for lat, lon, description in test_locations:
            ...
            try:
                ...
                print(f"  ✓ Cell {cell_idx} ({description}): "
        ...
        assert success_count == len(results), f"Some cells failed to load: {len(results) - success_count} failures"
...
class TestICONETOPOVisualization:
    ...
    def test_visualize_feature(self, setup):
        ...
        topo = var.topo_cell()
        etopo_reader = reader.read_etopo_topo(None, params, is_parallel=True, verbose=True)
        etopo_reader.get_topo(topo)
        etopo_reader.close_cached_files()
        topo.gen_mgrids()
        ...
        levels = 20
        cs = axes[1].contourf(topo.lon_grid, topo.lat_grid, topo.topo,
